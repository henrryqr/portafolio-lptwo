...
class FigurasApp:
    def __init__(self, root):
        ...
        # Variables para animación y resultados
        self.puntos_totales = 0
        self.calculos_realizados = 0
        self.area_actual = None
        self.perimetro_actual = None
    ...
    def vista_previa(self, event=None):
        ...
        self.dibujar_figura(es_calculo=False)
    
    def dibujar_figura(self, es_calculo=False):
        """Dibuja la figura en el canvas con o sin resultados"""
        self.canvas.delete('all')
            ...
            if figura == "rectangulo":
                ...
                if base_str and altura_str:
                    ...
                    if base > 0 and altura > 0:
                        ...
                        escala = min(220 / max_dim, 240 / max_dim)
                        
                        ancho = base * escala
                        alto = altura * escala
                        
                        # Ajustar posición si hay resultados
                        offset_y = 40 if es_calculo and self.area_actual else 0
                        
                        x1 = 160 - ancho / 2
                        y1 = 200 - alto / 2 + offset_y
                        x2 = x1 + ancho
                        y2 = y1 + alto
                        ...
                        self.canvas.create_text(x1 - 30, (y1+y2)/2, 
                                               text=f"h = {altura}", 
                                               font=('Comic Sans MS', 9, 'bold'),
                                               fill='#1976D2')
                        
                        # Mostrar resultados si es un cálculo
                        if es_calculo and self.area_actual is not None:
                            # Cuadro de resultados en la parte superior
                            self.canvas.create_rectangle(60, 20, 260, 75, 
                                                        fill='#FFE082', 
                                                        outline='#F57C00', 
                                                        width=3)
                            
                            self.canvas.create_text(160, 35, 
                                                   text="📊 RESULTADOS", 
                                                   font=('Comic Sans MS', 10, 'bold'),
                                                   fill='#E65100')
                            
                            self.canvas.create_text(160, 52, 
                                                   text=f"Área = {self.area_actual:.2f} u²", 
                                                   font=('Arial', 10, 'bold'),
                                                   fill='#D84315')
                            
                            self.canvas.create_text(160, 67, 
                                                   text=f"Perímetro = {self.perimetro_actual:.2f} u", 
                                                   font=('Arial', 10, 'bold'),
                                                   fill='#D84315')
            else:
                ...
                if radio_str:
                    ...
                    if radio > 0:
                        ...
                        escala = min(100 / radio, 120 / radio)
                        r = radio * escala
                        
                        # Ajustar posición si hay resultados
                        offset_y = 40 if es_calculo and self.area_actual else 0
                        center_y = 200 + offset_y
                        
                        self.canvas.create_oval(160 - r, center_y - r, 
                                               160 + r, center_y + r,
                                               fill='#81C784', 
                                               outline='#388E3C', 
                                               width=4)
                        ...
                        # Dibujar radio
                        self.canvas.create_line(160, center_y, 160 + r, center_y,
                                               fill='#D32F2F', width=3,
                                               arrow=tk.LAST)
                        
                        self.canvas.create_text(160 + r/2, center_y - 15, 
                                               text=f"r = {radio}", 
                                               font=('Comic Sans MS', 10, 'bold'),
                                               fill='#D32F2F')
    ...
    def calcular(self):
        ...
        try:
            ...
            if figura == "rectangulo":
                ...
                self.perimetro_actual = perimetro
                ...
                self.dibujar_figura(es_calculo=True)
